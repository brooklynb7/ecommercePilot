(function() {
	...
	app.controller('BrandDetailCtrl', ['$scope', 'BrandService', '$filter', '$stateParams', '$modal',
		function($scope, BrandService, $filter, $stateParams, $modal) {
			...
			BrandService.getBrand($stateParams.brandId).then(function(brand) {
				...
				if ($scope.city) {
					...
					_.each($scope.brand.selling_cities, function(city) {
						city.point = $scope.map.generatePoint(city, $scope.selectCity, $scope.viewCity);
					});
				...
				if ($scope.expandingCity) {
					...
					_.each($scope.brand.expanding_cities, function(city) {
						city.point = $scope.expandingCityMap.generatePoint(city, $scope.selectExpandingCity, null);
					});
					$scope.expandingCityMap.centerToPoint($scope.expandingCityMap.points[0]);
				}
			});
			...
			$scope.changeProductShowCity = function(){
				$scope.productShowCity = !$scope.productShowCity;
			};
